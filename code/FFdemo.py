...
def create_parameters(dt=0.001):
	...
	p = {'network_size': 300,  			# Number of units in network
		...
		'ff_steps_per_update': 1 ,  # Average number of steps per weight update
...
class RNN:
	...
	def run(self,inputs, record_flag=0, mode='train'):
		...
		def rnn_update(inp, activity):
			...
			res = activity + dx
			if mode == 'test': self.ICs.append(res)
			return res
	...
	def train(self,inps_and_targs, monitor_training=0, context=0, **kwargs):
		...
		*****NOTE***** The function inps_and_targs must have a third output of "hints" for training.
		...
		try:
			D_inputs, D_targs, D_hints = inps_and_targs(dt=p['dt'], context=context, **kwargs)[0:3]
		...
		for i in range(p['ff_init_trials']):
			print('.')
			print(context)
			inp, targ, hints = inps_and_targs(dt=p['dt'], context=context, **kwargs)[0:3]
			print(inp.shape, targ.shape, hints.shape)
		...
		for batch in range(p['ff_num_batches']):
			print('Batch %g of %g, %g trials: ' % (batch+1, p['ff_num_batches'], p['ff_trials_per_batch']))
			for trial in range(p['ff_trials_per_batch']):
				if np.mod(trial,50)==0:
					print('')
				print('.')
				# Create input, target, and hints. Combine for the driven network
				inp, targ, hints = inps_and_targs(dt=p['dt'], context=context, **kwargs)[0:3]  # Get relevant time series
				D_total_inp = np.hstack((inp,targ,hints))
				# For recording:
	...
	def test(self, inps_and_targs, context=0, **kwargs):
		p = self.p
		'''
		...
		for i in range(p['test_init_trials']):
			print('.')
			inp, targ = inps_and_targs(dt=p['dt'], context=context, **kwargs)[0:2]
			self.run(inp)
		print('')
		...
		for idx in range(p['test_trials']):
			...
			out = self.run(inp, mode='test')[0]
			plt.figure(idx)
			plt.imshow(out.T)
			self.save = out.T
			# line_out.set_xdata(tvec)
			# line_out.set_ydata(out)
			plt.title('RNN Testing, trial %g' % (idx+1))
			test_fig.canvas.draw()
