...
class NoisyNetworkMOIOC:
    ...
    def __init__(self, time_constant, num_neurons, p, chaotic_constant,
        ...
        self.color_data  = np.random.randn(num_neurons)
        self.motion_data = np.random.randn(num_neurons)
        self.color_cue   = np.random.randn(num_neurons)
        self.motion_cue  = np.random.randn(num_neurons)
        self.ret_cue     = np.random.randn(num_neurons)

        ###########     NOT A GENERAL SOLUTION     ###########
        """

        # experiment
        self.input = np.random.randn(input_num, num_neurons)
    ...
    def prop(self, z, r, context, Conv=1):
